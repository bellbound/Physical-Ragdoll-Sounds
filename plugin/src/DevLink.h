...
// never touches winsock: it appends finished frames to a staging buffer and
// reads back whatever the testbench pushed, both under one short mutex. If the
// testbench is not running, or goes away mid-session, nothing here says a word -
// see Link.h for why that silence is deliberate.
//
// ── the two directions ───────────────────────────────────────────────────────
//
// Out: every FeedEvent the engine drains, plus one profile per tracked actor and
// a status heartbeat. The events are teed off the feed rather than published
// separately - see LinkTap below - so what the testbench replays is byte for
...
public:
    ...
    /// Spawn the socket thread. A no-op when `config.enabled` is false, which is
    /// the shipping case: with the flag off this class makes no socket call at
    /// all and costs one branch a frame.
    ...
    /// The cell the player is in, for the sidecar the testbench writes. Cheap
    /// enough to set every frame; only the last value before a profile goes out
    /// is used.
    ...
    /// Frames the staging buffer had to throw away because the testbench was not
    /// draining fast enough. Non-zero means the cap below is wrong, and it is
    /// worth one log line at idle rather than a warning per frame.
...
private:
    ...
    /// How much unsent traffic is allowed to pile up before frames start being
    /// dropped.
    ///
    /// Four megabytes is about ten seconds of a busy knockdown at 128 bytes an
    /// event. Past that the testbench is not reading, and holding the backlog
    /// only turns a stalled tool into a growing heap inside Skyrim.
...
/// each drained batch into the link on the way past.
///
/// A wrapper rather than a hook inside GameFeed because the engine is the one
/// thing that decides what a frame's events *are*: it calls Drain with the tick's
/// clock, and anything published on a different schedule would be a second
/// opinion about the same stream.
